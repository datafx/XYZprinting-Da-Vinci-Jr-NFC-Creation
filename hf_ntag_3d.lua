...
local function write_tag(uid, t)
    ...
    for i= 8, 23 do
        cmd = ('hf mfu wrbl --block %02d -d %s -k %08X'):format(i, t[i], pwd)
        core.console(cmd)
    end

    --cfg1
    core.console(('hf mfu wrbl --block 42 -d %s -k %08X'):format(t[42], pwd))
    --cfg0
    core.console(('hf mfu wrbl --block 41 -d %s -k %08X'):format(t[41], pwd))
    --dynamic
    core.console(('hf mfu wrbl --block 40 -d %s -k %08X'):format(t[40], pwd))
